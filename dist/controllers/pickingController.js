...
class PickingController {
    ...
    static async assignWaves(req, res) {
        try {
            ...
            const unassignedWaves = await models_1.PickingWave.findAndCountAll({
    ...
    static async startPicking(req, res) {
        try {
            ...
            const picklistItems = await models_1.PicklistItem.findAndCountAll({
                where: { waveId },
    ...
    static async getExpiryAlerts(req, res) {
        try {
            ...
            const expiringItems = await models_1.PicklistItem.findAndCountAll({
                where: {
                    expiryDate: {
    ...
    static async getPicklistItems(req, res) {
        try {
            const { waveId } = req.params;
            ...
            if (!canView) {
                return responseHandler_1.ResponseHandler.error(res, 'Insufficient permissions to view this wave', 403);
            }
            const whereClause = { waveId: parseInt(waveId) };
            if (status)
                whereClause.status = status;
            const picklistItems = await models_1.PicklistItem.findAndCountAll({
                where: whereClause,
