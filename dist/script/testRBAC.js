...
async function testRBAC() {
    try {
        ...
        const rolePermissions = await models_1.RolePermission.findAll();
        ...
        if (adminRole) {
            const adminPermissions = await models_1.RolePermission.findAll({
                where: { roleId: adminRole.id }
            });
            console.log(`  - Admin permissions: ${adminPermissions.length} (should have all)`);
        }
        if (whManagerRole) {
            const whManagerPermissions = await models_1.RolePermission.findAll({
                where: { roleId: whManagerRole.id }
            });
            console.log(`  - WH Manager permissions: ${whManagerPermissions.length} (should have 14)`);
        }
        if (storeOpsRole) {
            const storeOpsPermissions = await models_1.RolePermission.findAll({
